...
setShowAiModal(true);
setAiAdvice(””);
const summary = ` 本月收入: ${fmt(income)} 本月支出: ${fmt(expense)} 本月結餘: ${fmt(income-expense)} 總負債: ${fmt(totalDebt)} 負債明細: ${debts.map(d=>`${d.name} 餘額${fmt(d.balance)} 年利率${d.rate}%`).join("、")} 儲蓄目標: ${goals.map(g=>`${g.name} 目標${fmt(g.target)} 已存${fmt(g.saved)}`).join("、")} 支出分類: ${catData.map(c=>`${c.name} ${fmt(c.value)}`).join("、")} `;
try {
